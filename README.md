...
SHARPpy maintains a variety of different sounding data sources in the distributed program.  These sounding data sources are provided by a variety of hosts online.  Accessiblity to these various data sources are dependent upon the hosts.
...
SHARPpy also links into the BUFKIT files that the NOAA/National Weather Service produces to distribute NWP point forecast soundings.  Currently SHARPpy links into the [Penn State Bufkit Distribution System](http://www.meteo.psu.edu/bufkit/) to access point forecast soundings for the GFS, NAM, NAM 3-km, SREF, RAP, HIRESW, and HRRR models run at NCEP.  The vertical grid for these data products are derived straight from the model native grid, as opposed to the isobaric levels that have been generated by post-processing software.
